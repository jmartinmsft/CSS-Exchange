...
function CreateService($smtpAddress, $impersonatedAddress = "") {
    ...
    if ([System.String]::IsNullOrEmpty($CertificateThumbprint)) {
        $Secret = ConvertFrom-SecureString $OAuthClientSecret -AsPlainText
...
Function SearchMailbox() {
	$ewsService = CreateService($Mailbox)
...
function GetWellKnownFolderPath($WellKnownFolder) {
    ...
    $folder = $null
    $folderPath = $null
    $folder = [Microsoft.Exchange.WebServices.Data.Folder]::Bind($ewsService,$WellKnownFolder)
    if ($folder) {
        $folderPath = GetFolderPath($folder)
...
Function SearchFolder( $FolderId ) {
    ...
    $folder = $null
    $folder = [Microsoft.Exchange.WebServices.Data.Folder]::Bind($ewsService,$FolderId)
	...
	while ($moreItems) {
        $results = $ewsService.FindItems( $FolderId, $searchFilter, $view )
